 ...
 */
#ifndef SVDQ_W4A8_DEBUG_READBACK_TORCH_ADPT_H
#define SVDQ_W4A8_DEBUG_READBACK_TORCH_ADPT_H

#include <torch/extension.h>

#include "../dispatch_ffn_combine_w4_a8/op_host/op_api/aclnn_svdq_w4a8_debug_readback.h"
...
inline const at::Tensor& first_svdq_w4a8_debug_tensor(const at::TensorList& tensors, const char* name)
{
    TORCH_CHECK(!tensors.empty(), name, " must not be an empty tensor list.");
    return tensors[0];
}

inline void check_svdq_w4a8_debug_rank(const at::Tensor& tensor, int64_t rank, const char* name)
{
...
std::tuple<at::Tensor, at::Tensor, at::Tensor, at::Tensor> svdq_w4a8_debug_readback(
    const at::Tensor& x,
    ...
    double swiglu_limit)
{
    check_svdq_w4a8_debug_rank(x, 2, "x");
    check_svdq_w4a8_debug_rank(expert_idx, 2, "expert_idx");
    check_svdq_w4a8_debug_rank(probs, 2, "probs");
    ...
    if (x_active_mask.has_value()) {
        check_svdq_w4a8_debug_rank(x_active_mask.value(), 1, "x_active_mask");
    ...
    const at::Tensor& residual_w1 = first_svdq_w4a8_debug_tensor(weight1, "weight1");
    const at::Tensor& residual_w2 = first_svdq_w4a8_debug_tensor(weight2, "weight2");
    const at::Tensor& residual_scale1 = first_svdq_w4a8_debug_tensor(scale1, "scale1");
    const at::Tensor& residual_scale2 = first_svdq_w4a8_debug_tensor(scale2, "scale2");
    const at::Tensor& residual_bias1 = first_svdq_w4a8_debug_tensor(bias1, "bias1");
    const at::Tensor& residual_bias2 = first_svdq_w4a8_debug_tensor(bias2, "bias2");
    check_svdq_w4a8_debug_rank(residual_w1, 3, "weight1");
    check_svdq_w4a8_debug_rank(residual_w2, 3, "weight2");
...
#endif  // SVDQ_W4A8_DEBUG_READBACK_TORCH_ADPT_H
